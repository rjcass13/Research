...
calc_latent_distance <- function(A_W, A_H, L_W, L_H) {
  ...
  # 2. Place points at cell centers so the covered area is exactly [0,1]x[0,1].
  # Each of the full_L_W columns gets width 1/full_L_W; its center is at (i - 0.5)/full_L_W.
  l_pts_norm = l_pts
  l_pts_norm[,1] = (l_pts[,1] - 1) / (full_L_W - 1) # Normalize X to [0,1]
  l_pts_norm[,2] = (l_pts[,2] - 1) / (full_L_H - 1) # Normalize Y to [0,1]
  l_pts_norm[,1] = (l_pts[,1] - 0.5) / full_L_W
  l_pts_norm[,2] = (l_pts[,2] - 0.5) / full_L_H
...
# MATERN: Agg to Obs #
######################
# Run simulations for a set of true values
...
# Run one simulation: makes true data then aggregates it to the different levels
run_one_sim <- function(j, aerial_dims, latent_dims, K_vec, l_D, log_params, chol_true, sim_true_s2, sim_true_rho, sim_true_nu, progress_file) {
  ...
  sim_rows <- matrix(NA, nrow = length(aerial_dims), ncol = 13)
  for (i in seq_along(aerial_dims)) {
    ...
    val <- optim(
      ...
      method = "L-BFGS-B", lower = c(-10, -10, -10), upper = c(3, 3, 3),
      y_block = y_obs, K = K, l_D = l_D, param_list = c(0, 0, 0)
    )
    sim_duration <- as.numeric(Sys.time() - sim_start_time, units = "secs")

    # Duration, TotalPoints, A_W, A_H, L_W, L_H, true_s2, true_rho, true_nu, est_s2, est_rho, est_nu, total_variance
    sim_rows[i, ] <- c(
...
# CAR: Agg to Obs #
###################
run_one_sim_icar <- function(j, W, aerial_dims, latent_dims, K_vec, l_D, log_params, progress_file) {

  cat(sprintf("[Sim %d] Started.\n", j), file = progress_file, append = TRUE)

  # Correctly draw from the iCAR using the eigendecomposition approach
...
test_sig_icar <- function(W, n_sim, progress_file, log_params, aerial_dims, latent_dims, K_vec, l_D, cl) {
  cat("[Covariance: iCAR] Started.\n", file = progress_file, append = TRUE)

  # Export W once to all workers rather than serializing it per-call
  clusterExport(cl, "W", envir = environment())

  results_list <- parLapply(cl, 1:n_sim, run_one_sim_icar,
    W = W, aerial_dims = aerial_dims, latent_dims = latent_dims,
    K_vec = K_vec, l_D = l_D, log_params = log_params,
    progress_file = progress_file)

  res <- as.data.frame(do.call(rbind, results_list))
  cat("[Covariance: iCAR] Complete.\n", file = progress_file, append = TRUE)
  res
}
...
# MATERN: Gen at Obs #
######################
mat_gen_at_obs_test_true_vals <- function(true_vals, n_sim, progress_file, log_params, aerial_dims, latent_dims, K_vec, l_D_vec, cl) {
  # True Data
  sim_true_s2 <- true_vals[[1]]
  sim_true_rho <- true_vals[[2]]
  sim_true_nu <- true_vals[[3]]
  ...
  results_list <- parLapply(cl, 1:n_sim, mat_gen_at_obs_run_one_sim, 
    aerial_dims = aerial_dims, latent_dims = latent_dims, 
    K_vec = K_vec, l_D_vec = l_D_vec, log_params = log_params,
    sim_true_s2 = sim_true_s2, sim_true_rho = sim_true_rho, sim_true_nu = sim_true_nu,
    progress_file = progress_file)
  # Combine: each list element is a 9-row matrix; rbind all 50 → 450 rows
  res <- as.data.frame(do.call(rbind, results_list))
...
# Run one simulation: makes true data then aggregates it to the different levels
mat_gen_at_obs_run_one_sim <- function(j, aerial_dims, latent_dims, log_params, K_vec, l_D_vec, sim_true_s2, sim_true_rho, sim_true_nu, progress_file) {
  ...
  for (i in seq_along(aerial_dims)) {
    ...
    sig_true <- mk_cov(sim_true_s2, sim_true_rho, sim_true_nu, l_D)
    chol_true <- chol(sig_true)
    y_obs <- as.vector(crossprod(chol_true, rnorm(nrow(chol_true))))
