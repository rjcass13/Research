source("~/GitRepos/Research/CoS/funcs.R")
library(mvtnorm)
library(ggplot2)
library(patchwork)
library(parallel)
...
l_D_vec <- mapply(function(A, L) calc_latent_distance(A, A, L, L), aerial_dims, latent_dims, SIMPLIFY = FALSE)
...
clusterEvalQ(cl, { source("~/GitRepos/Research/CoS/funcs.R"); library(mvtnorm) })

# Run the simulation
final_list <- apply(true_vals, 1, mat_gen_at_obs_test_true_vals, 
  progress_file = progress_file, n_sim = n_sim, log_params = log_params,
  aerial_dims = aerial_dims, latent_dims = latent_dims, 
  K_vec = K_vec, l_D_vec = l_D_vec, cl = cl)
...
write.csv(final_list, "square_48_sims_1.csv", row.names = FALSE)
